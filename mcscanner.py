...
print('Multithreaded mass minecraft server status checker by Footsiefat/Deathmonger, edited by @Duplexes#5466')
masscan = []
print("Loading servers...")
deta = Deta(input("Deta key: "))
print("Starting threads...")
print("Started! Keep this window open!")
db = deta.Base("Mcscanner")
...
def print_time(threadName):
    for z in split[int(threadName)]:
        ...
        try:
            ...
            print("Found server: " + ip + " Version: " + status.version.name + " Online: " + str(status.players.online))
            db.put({"ip": ip, "version": status.version.name, "online": status.players.online,
                    "players": query.players.names, "motd": query.motd})
